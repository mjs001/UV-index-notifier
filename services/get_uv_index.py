...
def uv_index_forecast_request(coordinates):
...
def get_current_uv_index(coordinates):
  url = f"https://api.openuv.io/api/v1/uv?lat={coordinates['lat']}&lng={coordinates['lon']}"
  try:
    res = requests.get(url, headers=headers)
    res.raise_for_status()
    uv_index_data = res.json()
    uv_index_data_dict = json.loads(uv_index_data)
    uv_index = uv_index_data["uv"]
   # date = need to look at what field is called
   # return date, uv_index
  except requests.exceptions.RequestException as e:
    print("An error occurred while attempting to retrieve the current UV index", e)
